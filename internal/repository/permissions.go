...
func (fr *FasterdogRepository) GetPermissions(ctx context.Context, userName, domainName string) ([]byte, error) {
	query := `
	WITH user_info AS (
        SELECT
            id,
	...
	err := fr.db.Pool().QueryRow(ctx, query, userName, domainName).Scan(&v1)
	if err != nil {
		return nil, err
	}
